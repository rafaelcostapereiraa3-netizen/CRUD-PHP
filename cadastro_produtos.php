...
if($_SERVER['REQUEST_METHOD'] === 'POST' && $csrf_valido && ($_POST['acao'] ?? '') ==='criar_categoria'){
    ...
    if($nome === ''){
        ...
        if($resultado['status'] === 201){
            ...
            $mensagem = "Sem permissão para cadastrar
            categoria (crie uma política de RLS de INSERT para o papel anon).";
            $tipo_mensagem = "erro";
        }        
        else{
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* form inline usado nos botões Excluir dentro da tabela — não pode quebrar o layout da linha */
        ...
        .acoes { display: flex; gap: 0.5rem; align-items: center; } /* agrupa os botões Editar/Excluir lado a lado */
        ...
        .topo-usuario { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem; font-size: 0.9rem; color: #4a5568; }
    </style>
</head>
<body>
    <div class="container">
        <div class="topo-usuario">
            <!-- $_SESSION['usuario_nome'] foi gravado no login.php após password_verify() confirmar a senha -->
        ...
        <p class="subtitulo">Sistema de administração de produtos e categorias: Create, Read, Update, Delete via API REST, protegido com token CSRF.</p>


        <?php if ($mensagem): ?>
            <!-- Só exibe o alerta se $mensagem não estiver vazia (definida por alguma ação acima) -->
            <!-- htmlspecialchars() aqui é o ÚNICO ponto de escape de $mensagem (defesa XSS, slide 5) -->
        ...
        <div class="card">
            ...
            <form method="POST">
                <!-- DEFESA CSRF (slide 6): token embutido como campo oculto — o servidor confere
                     esse valor contra o que está salvo em $_SESSION antes de processar o POST -->
                    ...
                    <tbody>
                        <?php foreach ($categorias as $categoria): ?>
                            ...
                            <tr>
                                ...
                                <td class="acoes">
                                    <!-- Editar continua sendo um link GET normal: só LÊ dados, não muda nada -->
                                    <a class="btn" href="listar_produtos.php?acao=editar_categoria&id=<?= (int) $categoria['id'] ?>">Editar</a>
                                    <!-- Excluir agora é um <form> POST com token CSRF, em vez de um link GET —
                                         um link GET poderia ser disparado sem o usuário perceber (ex: <img src="...">) -->
                                    <form method="POST" class="form-inline"
                                          onsubmit="return confirm('Excluir esta categoria? Só funciona se nenhum produto estiver usando ela.')">
                                        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token']) ?>">
        ...
        <div class="card">
            ...
            <form method="POST">
                <!-- DEFESA CSRF: mesmo token de sessão validado no PHP acima -->
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token']) ?>">
        ...
        <div class="card">
            ...
            <!-- method="GET" manda o termo de busca como parâmetro na URL (?busca=...),
                 permitindo recarregar a página com o filtro aplicado; é só leitura, então não
                 precisa de token CSRF -->
                        ...
                        <?php foreach ($produtos as $produto): ?>
                            ...
                            <tr>
                                ...
                                <td class="acoes">
                                    ...
                                    <!-- Excluir como <form> POST + token CSRF — mesma defesa aplicada em categoria -->
                                    <form method="POST" class="form-inline"
        ...
        <div class="card">
            ...
            <ul style="padding-left: 1.25rem;">
                ...
                <li>Adicionar autenticação (login) antes de liberar acesso a este painel — hoje qualquer pessoa com a URL pode usá-lo.</li>
